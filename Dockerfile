...
ENV ppm_version 1.6
RUN apt-get -y update && apt-get -y install build-essential libcrack2-dev wget libdb-dev libldap2-dev  && \
    wget ftp://ftp.openldap.org/pub/OpenLDAP/openldap-release/openldap-$ldap_version.tgz && \
    ...
    apt-get -y purge build-essential libcrack2-dev wget libdb-dev libldap2-dev && apt-get -y autoremove && \
    rm -rf /openldap-$ldap_version
